...
class Downloader {
  ...
  _fetchText(url, redirectsLeft = 5) {
    return new Promise((resolve, reject) => {
      ...
      const req = mod.get(url, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          res.resume();
          this._fetchText(res.headers.location, redirectsLeft - 1).then(resolve, reject);
          return;
        }
        let data = '';
  ...
  _downloadFile(url, destPath, redirectsLeft = 5) {
    return new Promise((resolve, reject) => {
      const mod = url.startsWith('https') ? https : http;
      const req = mod.get(url, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          if (redirectsLeft <= 0) return reject(new Error('Too many redirects'));
          res.resume();
          this._downloadFile(res.headers.location, destPath, redirectsLeft - 1).then(resolve, reject);
          return;
        }
      ...
      req.on('error', reject);
      req.setTimeout(300000, () => { req.destroy(); reject(new Error('File download timed out')); });
    });
  }
