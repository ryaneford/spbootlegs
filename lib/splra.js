...
async function fetchTorrentFile(torrentUrl, maxRedirects = 5) {
  return new Promise((resolve, reject) => {
    const httpModule = torrentUrl.startsWith('https') ? https : http;

    const handler = (res) => {
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        if (maxRedirects <= 0) return reject(new Error('Too many redirects'));
        fetchTorrentFile(res.headers.location, maxRedirects - 1).then(resolve, reject);
        res.resume();
        return;
      }
      const chunks = [];
    ...
    const req = httpModule.get(torrentUrl, handler);
    req.on('error', (err) => reject(new Error(`Failed to download torrent file: ${err.message}`)));
    req.setTimeout(30000, () => {
      req.destroy();
      reject(new Error('Torrent file download timed out'));
    });
  });
}

async function fetchArchiveMetadata(identifier) {
  const url = `https://s3.us.archive.org/${identifier}/${identifier}_meta.xml`;
  ...
  function fetchText(url, redirectsLeft = 5) {
    return new Promise((resolve, reject) => {
      ...
      req.on('error', (err) => reject(new Error(`Failed to fetch archive.org metadata: ${err.message}`)));
      req.setTimeout(15000, () => { req.destroy(); reject(new Error('Archive.org metadata request timed out')); });
    });
  }
  ...
  return {
    ...
    torrentUrl: `https://s3.us.archive.org/${identifier}/${identifier}_archive.torrent`,
  };
}
...
async function fetchArchiveTrackTitles(identifier) {
  ...
  function fetchText(targetUrl, redirectsLeft = 5) {
    return new Promise((resolve, reject) => {
      ...
      const req = mod.get(targetUrl, { headers: { 'User-Agent': 'sp-bootleg-downloader/1.0' } }, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          if (redirectsLeft <= 0) return reject(new Error('Too many redirects'));
          res.resume();
          fetchText(res.headers.location, redirectsLeft - 1).then(resolve, reject);
      ...
      req.on('error', (err) => reject(new Error(`Failed to fetch files XML: ${err.message}`)));
      req.setTimeout(15000, () => { req.destroy(); reject(new Error('Files XML request timed out')); });
    });
  }
